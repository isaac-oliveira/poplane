...
const main = (version, buildNumber) => {
  ...
  const iosInfoPath = path.join(__dirname, '..', 'ios', 'tkslojista', 'Info.plist')
  const iosProjectPath = path.join(__dirname, '..', 'ios', 'tkslojista.xcodeproj', 'project.pbxproj')

  if (checkVersionFormat(version)) {
    updateVersion(packagePath, /("version": )"\d+\.\d+\.\d+"/, `$1"${version}"`)
    updateVersion(androidPath, /(versionName )"\d+\.\d+\.\d+"/, `$1"${version}"`)
    updateVersion(iosProjectPath, /(MARKETING_VERSION = )\d+\.\d+\.\d+/g, `$1${version}`)
    updateVersion(
      iosInfoPath,
  ...
  if (checkBuildNumber(buildNumber)) {
    ...
    updateVersion(iosProjectPath, /(CURRENT_PROJECT_VERSION = )\d+/g, `$1${buildNumber - 1}`)
    updateVersion(iosInfoPath, /(<key>CFBundleVersion<\/key>\n[\t ]+<string>)\d+(<\/string>)/, `$1${buildNumber - 1}$2`)
  }
