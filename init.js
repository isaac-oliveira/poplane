...
const main = async (dexprotector, fastlane) => {
  ...
  if (runBoth || fastlane) {
    const androidFastlane = projectPaths.getFastlanePath("android", false);
    const iosFastlane = projectPaths.getFastlanePath("ios", false);
  ...
  if (dexprotector && !fastlane) {
    const { dexprotectorFilePath } = projectPaths.getDexprotectorPath();
    const { apk, aab, playstore, ios } = dexprotectorFilePath;
    if (
      !(await checkExists(
        [apk, aab, playstore, ios],
        "Dexprotector já configurado. Deseja continuar? (y/n): "
      ))
    ) {
...
async function initFastlane(dexprotector) {
  ...
  const packageName = await io.question(
    "Nome do pacote (packageName/appIdentifier): "
  );
  ...
  let wantIntegrateDexprotector = "n";
  if (dexprotector) {
    wantIntegrateDexprotector = await io.question(
      "Deseja integrar o ofuscamento com o Dexprotector (via SSH) nos arquivos do Fastlane? (s/n): "
    );
  }

  io.log("\nCopiando arquivos do fastlane...\n");

  const withDexprotector = wantIntegrateDexprotector === "s";
  const data = { projectName, packageName, appleId, itcTeamId, teamId, gitUrl };
  for (platform of ["ios", "android"]) {
    const { fastlaneTemplatesPath, fastlaneFilesPath, fastlaneFolderPath } =
      projectPaths.getFastlanePath(platform, withDexprotector);
...
async function initDexprotector() {
  ...
  const YOUR_FIREBASE_CERTIFICATEFINGERPRINT = await io.question(
    "Certificado de impressão digital do firebase: "
  );
  const YOUR_PLAYSTORE_CERTIFICATEFINGERPRINT = await io.question(
    "Certificado de impressão digital da playstore: "
  );
  ...
  const { dexprotectorFilePath, dexprotectorTemplatePath } =
    projectPaths.getDexprotectorPath();
  const keys = Object.keys(dexprotectorFilePath);
  keys.forEach((key) => {
    const path = dexprotectorFilePath[key];
    if (path) {
      move(
        dexprotectorTemplatePath[key],
        dexprotectorFilePath[key],
        null,
        data,
        (key) => new RegExp(`${key}`, "g")
      );
  ...
  io.log(
    "Arquivos de configuração do dexprotector gerados com sucesso!",
    "green"
  );
}
